...
public class JsonPlayerPrefs
{
	...
	public float GetFloat(string key, float defaultValue = 0f)
	{
		...
		{
			float value;
			if (float.TryParse(playerPref.value, System.Globalization.NumberStyles.Any, CultureInfo.GetCultureInfo("en"), out value))
			{
				return value;
			}
		}
	...
	private string ReplaceValueCommasToPoints(StringBuilder str)
	{
		const int INDENT_TO_VALUE = 9;
		int i = 0;
		while (i < str.Length)
		{
			if (str[i] != '"')
